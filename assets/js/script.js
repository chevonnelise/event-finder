document.addEventListener("DOMContentLoaded", async function () {
    ...
    for (let venue of venueResponse.data.features) {
        ...
        const marker = L.marker(reversedCoordinates, { icon: venueIcon }).bindPopup(`<h3>${venue.properties.Stadium}</h3><h5>${venue.properties.Country} (${venue.properties["City/State"]})</h5>`).addTo(venueLayer);
